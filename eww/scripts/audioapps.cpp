...
void getAudioClients() {
    ...
    if (clients.empty()) {
        return;  // No active streams (or pactl unavailable) - leave apps empty
    }
    try {
        clientjson = json::parse(clients);
    } catch (const json::parse_error&) {
        return;  // Malformed/partial output - fail gracefully instead of crashing
    }
    for (json client : clientjson) {
        try {
            addApp(client);
        } catch (const json::exception&) {
            continue;  // Skip streams with unexpected shape (e.g. mono channel naming)
        }
        // cout << client << '\n';
    }
}
