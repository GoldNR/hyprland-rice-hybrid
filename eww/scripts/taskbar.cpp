...
void addApp(json& client) {
    ...
    for (json& obj : apps) {
        ...
        if (it != obj.end() && *it == client["class"]) {
            ...
            obj["title"].push_back(client["title"]);
            break;
        }
    }
    if (!found) {
        json newApp =
            R"({"class": "", "count": 1, "workspace": [], "address": [], "title": [], "icon": ""})"_json;
        newApp["class"] = client["class"];
        newApp["address"].push_back(client["address"]);
        newApp["workspace"].push_back(client["workspace"]["id"]);
        newApp["title"].push_back(client["title"]);
...
void getAppNameAndCount() {
    ...
    if (clients.empty() || pinned.empty()) return;
    try {
